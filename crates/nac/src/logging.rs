...
pub fn list_log_files() -> std::io::Result<Vec<PathBuf>> {
    let Some(logs_dir) = crate::paths::nac_logs_dir() else {
        return Ok(Vec::new());
    };
...
mod tests {
    ...
    fn list_log_files_and_tail_current_log_lines_work() {
        ...
        let path = crate::paths::nac_log_path_for_pid(std::process::id()).unwrap();
        std::fs::write(root.join("logs/nac-old.log"), "old\n").unwrap();
    ...
    fn rotate_current_log_if_too_large_renames_file() {
        ...
        std::fs::create_dir_all(&root).unwrap();
        let current = root.join("nac-123.log");
        ...
        assert!(rotated[0].starts_with("nac-123-"));
    ...
    fn prune_logs_older_than_removes_stale_logs_but_keeps_current() {
        ...
        std::fs::create_dir_all(&root).unwrap();
        let current = root.join("nac-100.log");
        let stale = root.join("nac-101.log");
